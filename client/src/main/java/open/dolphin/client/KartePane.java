...
import jakarta.imageio.stream.FileImageInputStream;
import jakarta.swing.*;
import jakarta.swing.event.CaretEvent;
import jakarta.swing.event.CaretListener;
...
//s.oh$
    ...
    protected JPopupMenu createMenus() {
        ...
        if (getTextPane().isEditable()) {
            ...
            ccl.addPropertyChangeListener(ColorChooserComp.SELECTED_COLOR, (PropertyChangeEvent e) -> {
            ...
            JLabel l = new JLabel(ClientContext.getMyBundle(KartePane.class).getString("label.color.text"));
...
//masuda$
    ...
    protected void setBackgroundUneditable() {
        getTextPane().setBackground(GUIConst.KARTE_UNEDITABLE_BK_COLOR);
        getTextPane().setOpaque(true);
    }
...
//s.oh$
            ...
            public void succeeded(StampModel result) {
                if (result != null) {
                    try {
                        ...
                        IInfoModel model;
                        try (XMLDecoder d = new XMLDecoder(new BufferedInputStream(new ByteArrayInputStream(bytes)))) {
    ...
    private boolean showMaxSizeMessage() {
        ...
        java.util.ResourceBundle bundle = ClientContext.getMyBundle(KartePane.class);
        String width = bundle.getString("image.max.width");
        String height = bundle.getString("image.max.height");
        int maxImageWidth = Integer.parseInt(width);
        int maxImageHeight = Integer.parseInt(height);
        
        String title = bundle.getString("title.aboutImageSize");
        String msg = bundle.getString("message.aboutImageSize");
        MessageFormat msft = new MessageFormat(msg);
        JLabel msg1 = new JLabel(msft.format(new Object[]{maxImageWidth, maxImageHeight}));
        
        String question = bundle.getString("question.resizeImage");
        JLabel msg2 = new JLabel(question);
        
        String suppress = bundle.getString("suppress.imageSize.message");
        final JCheckBox cb = new JCheckBox(suppress);
        ...
        String optionResize = bundle.getString("option.resize");
        ...
        int option = JOptionPane.showOptionDialog(w,
                            ...
                            new String[]{optionResize, GUIFactory.getCancelButtonText()}, optionResize);
        return option == 0;       
    }
    ...
    private void showNoReaderMessage() {
        Window w = SwingUtilities.getWindowAncestor(getTextPane());
        java.util.ResourceBundle bundle = ClientContext.getMyBundle(KartePane.class);
        String msg = bundle.getString("warning.noAvailable.reader");
        String title = bundle.getString("title.imageImport");
    ...
    private void showFileInfo(final AttachmentModel attachment) {
        ...
        java.util.ResourceBundle bundle = ClientContext.getMyBundle(KartePane.class);
        if (canAttach) {
            String byesInfo = bundle.getString("info.fileSize");
            MessageFormat msft = new MessageFormat(byesInfo);
            view.getSizeLbl().setText(msft.format(new Object[]{String.valueOf(attSize)}));
        } else {
            String tooLarge = bundle.getString("warning.tooLargeFile");
            MessageFormat msft = new MessageFormat(tooLarge);
            view.getSizeLbl().setText(msft.format(new Object[]{Project.getString("attachment.max.size.display")}));
        ...
        view.getTitleFld().getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void insertUpdate(DocumentEvent de) {
                boolean enabled = (!view.getTitleFld().getText().equals(""));
        ...
        String title = bundle.getString("title.attacheOrInsertImage");
        final JDialog dialog = pane.createDialog(w, ClientContext.getFrameTitle(title));
    ...
    private String millisToTime(long l) {
        Date d = new Date(l);
        String timeFmt = ClientContext.getMyBundle(KartePane.class).getString("timeFormat.lastModified");
    ...
    public void fileDropped(final File file) {
        ...
        SwingWorker worker = new SwingWorker<AttachmentModel, Void>() {
            ...
            protected AttachmentModel doInBackground() throws Exception {
                ...
                byte[] data;
                try ( // Content を読み込む
                        FileInputStream in = new FileInputStream(file); FileChannel fChan = in.getChannel()) {
                    len = fChan.size();
...
//s.oh$
    ...
    public void removeStamp(StampHolder[] sh) {
        if (sh != null && sh.length > 0) {
            for (StampHolder sh1 : sh) {
//s.oh^ 2013/11/26 スクロールバーのリセット
                //removeStamp(sh[i]);
                removeStamp(sh1, false);
//s.oh$
            }
        }
    }
        ...
        if(list != null) {
            for(ModuleModel mm : list) {
                if(mm.getModel() instanceof BundleDolphin) {
                    String labTest = ClientContext.getMyBundle(KartePane.class).getString("orderName.labTest");
                    if(((BundleDolphin)mm.getModel()).getOrderName() != null && ((BundleDolphin)mm.getModel()).getOrderName().startsWith(labTest)) {
                        return true;
                    }
                }
            }
